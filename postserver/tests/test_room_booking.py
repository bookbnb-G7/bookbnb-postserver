...
def test_book_a_room_without_api_key(test_app):
    response = test_app.post(
        url="/rooms/" + str(room_id) + "/bookings/",
        data=json.dumps(test_room_booking_payload),
    )
...
def test_book_an_non_existent_room(test_app):
...
def test_get_a_room_booking_from_a_non_existen_room(test_app):
    booking_id = 1
    not_existent_room_id = 25
...
def test_delete_a_non_existent_room_booking(test_app):
