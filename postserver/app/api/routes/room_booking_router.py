...
@router.patch("/{room_booking_id}", response_model=RoomBookingDB, status_code=200)
async def patch_room_booking(
    room_id: int, room_booking_id: int,
    payload: RoomBookingPatch, db: Session = Depends(get_db),
    api_key: Optional[str] = Header(None)
):
    auth_service.verify_apy_key(api_key)
    room_booking_info = RoomBookingDAO.update_room_booking(db, room_id, room_booking_id, payload)
    return room_booking_info
