...
from app.api.models.room_rating_model import RoomRatingSchema, RoomRatingDB,\
											 RoomRatingList, RoomRatingPatch
...
async def delete_room_rating(room_id: int, rating_id: int, db: Session = Depends(get_db)):
...
async def update_room_rating(payload: RoomRatingPatch, room_id: int, rating_id: int, db: Session = Depends(get_db)):
