...
class RoomPhotoDAO:
	...
	def add_new_room_photo(cls, db, room_id, room_photo_args): 
		new_room_photo = RoomPhoto(room_id=room_id,
	...
	def get_room_photo(cls, db, firebase_id):
		room_photo = db.query(RoomPhoto).filter(RoomPhoto.firebase_id == firebase_id).first()
	...
	def get_all_room_photos(cls, db, room_id):
		room_photos_list = db.query(RoomPhoto).filter(RoomPhoto.room_id == room_id).all()
